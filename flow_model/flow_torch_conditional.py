...
class NormalizingFlow(nn.Module):
    ...
    def __init__(self, n_dim, n_cond_dim, n_units, rqs=None, lu_fact=None):
        """
...
#----------------------------------------------------------------------------------
    def build(self, n_dim, n_units, rqs=None, lu_fact=None):
        ...
        if rqs is None:
            self.nsf = [NSF_CL(dim=self._n_dim, cond_dim=self._n_cond_dim,\
                        K=8, B=3, hidden_dim=16)\
...
#----------------------------------------------------------------------------------
    def backward(self, z, x_cond):
        m, _ = z.shape
        log_det = torch.zeros(m).to(device)
        for flow in self.bij[::-1]:
            z, ld = flow.backward(z, x_cond)
            log_det += ld
        xs = z
        return xs, log_det

#----------------------------------------------------------------------------------
    def sample(self, sample_shape, x_cond):
        z = self.dist.sample(sample_shape)
        xs, _ = self.backward(z, x_cond)
        return xs
...
def plot_prob(bij_p, dist, ax_p=None, ax_lnp=None):
    ...
    log_det = torch.zeros(m).to(device)
    for flow in bij_p:
        xy_grid, ld = flow.forward(xy_grid, x_cond)
        log_det += ld
    zs = xy_grid
...
#----------------------------------------------------------------------------------
    def plot_fn():
        ...
        y_sample, _ = flow.backward(x_sample, x_cond)
...
#--------------------------------------------------------------------------------------------
def train_flow(flow, data, choose_cond, optimizer=None, batch_size=32,
...
#----------------------------------------------------------------------------------
    ...
    for e in range(int(n_epochs)):
        ...
        for batch_idx, data_batch in enumerate(train_loader):
            ...
            # conditional variable (or not)
            choose_not_cond = np.arange(x.shape[1])
            choose_not_cond = choose_not_cond[(np.invert(np.isin(choose_not_cond,choose_cond)))]

            zs, prior_logprob, log_det = flow(x[:,choose_not_cond],x[:,choose_cond])
...
class NSF_CL(nn.Module):
    ...
    def __init__(self, dim, cond_dim, K=8, B=3, hidden_dim=16, base_network=MLP):
        super().__init__()
        self.dim = dim
        self.K = K
        self.B = B
        ...
        self.g1 = base_network(cond_dim, (3 * K - 1) * dim // 2, hidden_dim)
        self.g2 = base_network(cond_dim, (3 * K - 1) * dim // 2, hidden_dim)

    def forward(self, x, x_cond):
        log_det = torch.zeros(x.shape[0]).to(device)
        lower, upper = x[:, :self.dim // 2], x[:, self.dim // 2:]
        out = (self.g1(x_cond)*self.f1(lower)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        ...
        out = (self.g2(x_cond)*self.f2(upper)).reshape(-1, self.dim // 2, 3 * self.K - 1)
        W, H, D = torch.split(out, self.K, dim = 2)
        W, H = torch.softmax(W, dim = 2), torch.softmax(H, dim = 2)
        W, H = 2 * self.B * W, 2 * self.B * H
        D = F.softplus(D)
