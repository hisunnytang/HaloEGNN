...
def obtain_condition_transformer(dl, max_samples=500, transform_type='power'):
    ...
    else transform_type == 'quantile':
        condition_normalizer = QuantileTransformer(output_distribution='normal').fit(targets)
...
def train_loop(
    ...
    for i in range(current_epoch, max_epochs):
        ...
        print(
            f"Epoch {i}: train loss = {loss.item():.2f}; val loss = {val.item():.2f}"
        )
        scheduler.step(val)
