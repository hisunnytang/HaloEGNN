...
def create_selectionrange_slider_map(cols_bins):
  ...
  for c, _bins in cols_bins:
    sliders_map[c] = widgets.SelectionRangeSlider(
      options=_bins,
      index=[3,5]
      )
  return sliders_map
...
def create_slider_map(cols_min_max):
  ...
  for c, _min, _max in cols_min_max:
    ...
    sliders_map[c] = widgets.FloatRangeSlider(
        min=_min, max=_max,step=(_max-_min)/100,
        )
  return sliders_map
...
def order_halo_by_mass(x, feat, mass_idx =0):
  ...
  x_sorted_by_mass = torch.stack([x[idx] for x, idx in zip(Xsamp,Msample_sorted_idx)])
  feat_sorted_by_mass = torch.stack([x[idx] for x, idx in zip(feat,Msample_sorted_idx)])
  return x_sorted_by_mass, feat_sorted_by_mass

def compute_metric_features(xposin, m, max_prog=3):
  ...
  for bsz in range(len(xpos)):
    # d = pairwise_distances(xpos[bsz,:max_prog,:])
  ...
  bsz = len(xpos)
  all_mass = m[:,:max_prog].reshape(len(xpos),-1).numpy()

  dij = np.triu_indices(max_prog,1)
  feat_names = [f"feat_{i}" for i in range(all_mass.shape[1])] + [f"log_d{i}{j}" for i,j in zip(*dij) ]
  # print(all_dist.shape, all_mass.shape)
  return np.hstack((all_mass, np.log(all_dist) )), feat_names
...
def get_dataloader_features(dl, max_progenitors=20, distance_norm=300):
  subset_all_feats = []
...
from HaloEGNN.HaloEGNNFlows.EGNNFlows.datasets import ProgenitorDataset

def obtain_conditional_feature_df(
    filelist, 
    condition_columns,
    initial_slice,
    final_slice,
  ...
  all_feats_np, cname = get_dataloader_features(
    dl, 
    max_progenitors, 
    distance_norm
  )
  ...
  return df_full_cond, df_sub_cond, df1, df2
